/**
 * LinkAccountButton - Card-style button to initiate OAuth flow for account linking
 *
 * Matches the ProfileImportSelector style with platform-branded cards.
 */
...
function ArrowRightIcon({ size = 18, color }: { size?: number; color: string }) {
  return (
...
export interface LinkAccountButtonProps {
  /** The platform to link (discord, github, or slack). */
  platform: Platform;
 ...
 */
const PLATFORM_CONFIG: Record<Platform, { name: string; color: string; darkColor: string; description: string }> = {
  discord: {
    name: 'Discord',
    color: '#5865F2',
    darkColor: '#5865F2',
    description: 'Link your Discord account for friend discovery',
  },
  github: {
    name: 'GitHub',
    color: '#24292e',
    darkColor: '#f0f6fc',
    description: 'Link your GitHub profile for friend discovery',
  },
  steam: {
    name: 'Steam',
    color: '#1b2838',
    darkColor: '#66c0f4',
    description: 'Link your Steam account for friend discovery',
  },
  bluesky: {
    name: 'Bluesky',
    color: '#0085FF',
    darkColor: '#0085FF',
    description: 'Link your Bluesky account for friend discovery',
  },
  xbox: {
    name: 'Xbox',
    color: '#107C10',
    darkColor: '#107C10',
    description: 'Link your Xbox account for friend discovery',
  },
};
...
export function LinkAccountButton({
  ...
  const platformColor = isDark ? config.darkColor : config.color;
        ...
        >
          <HStack gap="md" style={{ alignItems: 'center' }}>
            ...
            {loading ? (
              ...
              <ArrowRightIcon size={18} color={textMuted} />
            )}
          </HStack>
        </Card>
      )}
...
LinkAccountButton.displayName = 'LinkAccountButton';
