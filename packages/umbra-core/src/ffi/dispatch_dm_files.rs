...
use super::dispatcher::{
    DResult, err, json_parse, require_str, ok_json, ok_success, dm_file_service,
    dm_file_json, dm_folder_json, emit_event,
};
...
pub fn dm_upload_file(args: &str) -> DResult {
    ...
    let f = svc.upload_file(
        conversation_id, folder_id, filename, description,
        file_size, mime_type, storage_chunks_json, uploaded_by,
        encrypted_metadata, encryption_nonce,
...
pub fn dm_move_file(args: &str) -> DResult {
    ...
    let svc = dm_file_service()?;
    svc.move_file(id, target_folder_id).map_err(|e| err(e.code(), e))?;
    ok_success()
}
...
pub fn dm_create_folder(args: &str) -> DResult {
    ...
    let svc = dm_file_service()?;
    let f = svc.create_folder(conversation_id, parent_folder_id, name, created_by)
        .map_err(|e| err(e.code(), e))?;
