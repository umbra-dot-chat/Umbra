...
pub fn messaging_get_conversations() -> DResult {
    ...
    let arr: Vec<serde_json::Value> = convs
        .iter()
        .map(|c| {
            serde_json::json!({
...
pub fn messaging_mark_read(args: &str) -> DResult {
    ...
    super::dispatcher::emit_event(
        "message",
        &serde_json::json!({"type": "conversationRead", "conversation_id": conv_id}),
    );
...
pub fn messaging_edit(args: &str) -> DResult {
    ...
    if message.sender_did != our_did {
        return Err(err(403, "Not the sender"));
    }
    ...
    db.edit_message(msg_id, &ciphertext, &nonce.0, edited_at)
        .map_err(|e| err(e.code(), e))?;

    super::dispatcher::emit_event(
        "message",
        &serde_json::json!({"type": "messageEdited", "message_id": msg_id, "edited_at": edited_at}),
    );
...
pub fn messaging_delete(args: &str) -> DResult {
    ...
    db.delete_message(msg_id, deleted_at)
        .map_err(|e| err(e.code(), e))?;

    super::dispatcher::emit_event(
        "message",
        &serde_json::json!({"type": "messageDeleted", "message_id": msg_id}),
    );
...
pub fn messaging_pin(args: &str) -> DResult {
    ...
    db.pin_message(msg_id, &pinned_by, pinned_at)
        .map_err(|e| err(e.code(), e))?;

    super::dispatcher::emit_event(
        "message",
        &serde_json::json!({"type": "messagePinned", "message_id": msg_id}),
    );

    ok_json(
        serde_json::json!({ "message_id": msg_id, "pinned_by": pinned_by, "pinned_at": pinned_at }),
    )
}
...
pub fn messaging_unpin(args: &str) -> DResult {
    ...
    super::dispatcher::emit_event(
        "message",
        &serde_json::json!({"type": "messageUnpinned", "message_id": msg_id}),
    );
...
pub fn messaging_add_reaction(args: &str) -> DResult {
    ...
    db.add_reaction(&reaction_id, msg_id, &reactor_did, emoji)
        .map_err(|e| err(e.code(), e))?;
    ...
    super::dispatcher::emit_event(
        "message",
        &serde_json::json!({"type": "reactionAdded", "message_id": msg_id, "emoji": emoji}),
    );
...
pub fn messaging_remove_reaction(args: &str) -> DResult {
    ...
    let db = state
        ...
        .ok_or_else(|| err(400, "Database not initialized"))?;
    let reactor_did = identity.did_string();
    db.remove_reaction(msg_id, &reactor_did, emoji)
        .map_err(|e| err(e.code(), e))?;
    let reactions = db.get_reactions(msg_id).map_err(|e| err(e.code(), e))?;
    let arr: Vec<serde_json::Value> = reactions
        .iter()
    ...
    super::dispatcher::emit_event(
        "message",
        &serde_json::json!({"type": "reactionRemoved", "message_id": msg_id, "emoji": emoji}),
    );
...
pub fn messaging_forward(args: &str) -> DResult {
    ...
    let timestamp = crate::time::now_timestamp_millis();
    let ct_bytes = hex::decode(&msg.content_encrypted)
        .map_err(|e| err(704, format!("Invalid content: {}", e)))?;
    let nonce_bytes =
        hex::decode(&msg.nonce).map_err(|e| err(704, format!("Invalid nonce: {}", e)))?;
    db.store_message(
        &new_id,
        target_conv_id,
        &msg.sender_did,
    ...
    .map_err(|e| err(e.code(), e))?;
    ok_json(
        serde_json::json!({ "new_message_id": new_id, "target_conversation_id": target_conv_id }),
    )
}
...
pub fn messaging_update_status(args: &str) -> DResult {
    ...
    match status {
        "sent" => {
            db.mark_message_sent(msg_id).map_err(|e| err(e.code(), e))?;
        }
        "delivered" => {
        ...
        "read" => {
            db.mark_message_read(msg_id).map_err(|e| err(e.code(), e))?;
        }
    ...
    super::dispatcher::emit_event(
        "message",
        &serde_json::json!({"type": "messageStatusUpdated", "message_id": msg_id, "status": status}),
    );
