...
pub fn friends_send_request(args: &str) -> DResult {
    ...
    database.store_friend_request(&record).map_err(|e| err(e.code(), e))?;
    ...
    super::dispatcher::emit_event("friend", &serde_json::json!({"type": "friendRequestSent", "request_id": request.id, "to_did": request.to_did}));
...
pub fn friends_store_incoming(args: &str) -> DResult {
    ...
    db.store_friend_request(&record).map_err(|e| err(e.code(), e))?;

    super::dispatcher::emit_event("friend", &serde_json::json!({"type": "friendRequestReceived", "request_id": record.id, "from_did": record.from_did}));
...
pub fn friends_accept_request(args: &str) -> DResult {
    ...
    let req = db.get_friend_request(request_id)
        .map_err(|e| err(e.code(), e))?
    ...
    super::dispatcher::emit_event("friend", &serde_json::json!({"type": "friendAdded", "request_id": request_id, "friend_did": friend_did, "conversation_id": conv_id}));
...
pub fn friends_reject_request(args: &str) -> DResult {
    ...
    let state = state.read();
    let db = state.database.as_ref().ok_or_else(|| err(400, "Database not initialized"))?;
    db.update_request_status(request_id, "rejected").map_err(|e| err(e.code(), e))?;

    super::dispatcher::emit_event("friend", &serde_json::json!({"type": "friendRequestRejected", "request_id": request_id}));
...
pub fn friends_pending_requests(args: &str) -> DResult {
    ...
    let state = state.read();
    let db = state.database.as_ref().ok_or_else(|| err(400, "Database not initialized"))?;
    let reqs = db.get_pending_requests(direction).map_err(|e| err(e.code(), e))?;
...
pub fn friends_remove(args: &str) -> DResult {
    ...
    super::dispatcher::emit_event("friend", &serde_json::json!({"type": "friendRemoved", "did": did}));
...
pub fn friends_block(args: &str) -> DResult {
    ...
    super::dispatcher::emit_event("friend", &serde_json::json!({"type": "userBlocked", "did": did}));
