...
use umbra_core::friends::{Friend, FriendRequest, FriendResponse};
...
fn main() {
    ...
    println!(
        "   From: {} ({})",
        request.from.display_name, request.from.did
    );
    println!("   To: {}", request.to_did);
    println!("   Message: {:?}", request.message);
    println!(
        "   Signature (hex): {}...",
        &request.signature.to_hex()[..32]
    );
    println!();
    ...
    let request_json = request.to_json().expect("Failed to serialize request");
    ...
    let received_request =
        FriendRequest::from_json(&request_json).expect("Failed to deserialize request");
    ...
    if let Some(ref responder) = response.responder {
        println!(
            "   Responder: {} ({})",
            responder.display_name, responder.did
        );
    }
    println!();
    ...
    let received_response =
        FriendResponse::from_json(&response_json).expect("Failed to deserialize response");
    ...
    let bob_as_friend = Friend::from_public_identity(received_response.responder.as_ref().unwrap());
